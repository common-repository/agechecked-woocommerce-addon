...
{	
    ...
    class AgeChecked_WC_Addon extends WC_Settings_API
    {
		...
		public static function get_method_title() {
			return __("AgeChecked", 'agechecked-wc-addon');
		}
		
		public static function get_method_description() {
			return __("AgeChecked Plug-in for WooCommerce", 'agechecked-wc-addon');
		}		
        ...
        function __construct()
        {
            ...
            $this->method_title = self::get_method_title();
            
            // The description, shown on the actual Payment options page on the backend
            $this->method_description = self::get_method_description();
		...
		function agechecked_woocommerce_addon_checkout_update_order_meta($order_id)
		{
			if (!empty($_POST['agechecked_ageverifiedid'])) {
				...
				update_post_meta($order_id, 'ageverifiedid', '<span style="color:red">Not Verified</span>');
			}
		}
